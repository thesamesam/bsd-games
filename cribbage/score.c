...
// Score the given hand of n cards and the starter card. n must be <= 4
int scorehand (const CARD hand[], CARD starter, int n, bool crb, bool do_explain)
{
    int i, k;
    int score;
    bool flag;
    ...
    if (do_explain) {
	if (i > 0) {
	    (void) sprintf(buf, ", %d points in pairs, %d in runs", pairpoints, runpoints);
	    strcat(explan, buf);
	} else
...
int fifteens (const CARD hand[], int n)
{
    int *sp, *np;
    int i;
    const CARD *endp;
...
int pairuns (const CARD h[], int n)
{
    int i;
...
int pegscore (CARD crd, const CARD tbl[], int n, int sum)
{
    bool got[RANKS];
    int i, j, scr;
    int k, lo, hi;
...
int adjust (const CARD cb[], CARD tnv UNUSED)
{
    long scr;
    int i, c0, c1;
