...
static void getscores (FILE** fpp)
{
    int sd, mint, lck;
    mode_t mask;
    ...
    // Grab a lock.
    if (flock(sd, lck))
	warn("warning: score file %s cannot be locked", _PATH_SCOREFILE);
...
void savescore(int level)
{
	...
	if (score > sp->hs_score) {
	    (void) printf("%s bettered %s %d score of %d!\n", "\nYou", "your old level", level, sp->hs_score * sp->hs_level);
	...
	rewind(sf);
	if (fwrite(scores, sizeof(*sp), nscores, sf) != (size_t) nscores || fflush(sf) == EOF)
	    warnx("error writing %s: %s -- %s", _PATH_SCOREFILE, strerror(errno), "high scores may be damaged");
    }
...
static int cmpscores(const void *x, const void *y)
{
    ...
    long l = (long) b->hs_level * b->hs_score - (long) a->hs_level * a->hs_score;
    if (l < 0)
	return -1;
    if (l > 0)
	return 1;
...
static int checkscores(struct highscore *hs, int num)
{
    struct highscore *sp;
    int i, j, k, numnames;
    ...
    qsort (hs, nscores, sizeof(*hs), cmpscores);
	...
	if (j == numnames) {
	    ...
	    if ((pu->times < MAXSCORES && getpwnam(sp->hs_name) != NULL && sp->hs_time + EXPIRATION >= now) || levelfound[sp->hs_level] == 0)
		pu->times++;
	    else {
...
static void printem(int level, int offset, struct highscore *hs, int n, const char *me)
{
    struct highscore *sp;
...
#define	TITLE "Rank  Score   Name     (points/level)"
    ...
    // This makes a nice two-column sort with headers, but it's a bit convoluted...
    printf("%s   %s\n", TITLE, n > 1 ? TITLE : "");
	...
	for (col = 0; col < 2; col++) {
	    ...
	    sp = &hs[item];
	    sprintf(buf, "%3d%c %6d  %-11s (%6d on %d)", item + offset, sp->hs_time ? '*' : ' ', sp->hs_score * sp->hs_level, sp->hs_name, sp->hs_score, sp->hs_level);
	    // Highlight if appropriate.  This works because
	    // we only get one score per level.
	    if (me != NULL && sp->hs_level == level && sp->hs_score == score && strcmp(sp->hs_name, me) == 0) {
		putpad(SOstr);
		highlight = 1;
	    }
	    printf("%s", buf);
