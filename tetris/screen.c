...
struct tcsinfo {		// termcap string info; some abbrevs above
    ...
    { "bc", &bcstr}, { "ce", &CEstr}, { "cl", &CLstr}, { "cm", &CMstr},
#ifdef unneeded
    { "cr", &CRstr},
#endif
    { "le", &BC},		       // move cursor left one space
    { "pc", &pcstr}, { "se", &SEstr}, { "so", &SOstr}, { "te", &TEstr}, { "ti", &TIstr}, { "up", &UP},		       // cursor up
    { {0}, NULL}
};
...
void scr_init(void)
{
	...
	{ "co", &COnum}, { "li", &LInum}, { "sg", &sgnum}, { { 0}, NULL}
    };
    ...
    fill = combuf;
    for (struct tcsinfo* p = tcstrings; p->tcaddr; p++)
	*p->tcaddr = tgetstr(p->tcname, &fill);
    for (struct tcninfo* p = tcflags; p->tcaddr; p++)
	*p->tcaddr = tgetflag(p->tcname);
    for (struct tcninfo* p = tcnums; p->tcaddr; p++)
...
void scr_set(void)
{
	...
	(void) fprintf(stderr, "the screen is too small: must be at least %dx%d, ", MINCOLS, MINROWS);
	stop("");	       // stop() supplies \n
    }
