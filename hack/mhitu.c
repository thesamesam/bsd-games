...
int mhitu(struct monst *mtmp)
{
    ...
    ctmp = tmp && !mtmp->mcan && (!uarm || objects[uarm->otyp].a_can < rnd(3) || !rn2(50));
    switch (mdat->mlet) {
	case '1':
	    if (wiz_hit(mtmp))
	...
	case ';':
		...
		if (!u.ustuck && !rn2(10)) {
		    ...
		    u.ustuck = mtmp;
		} else if (u.ustuck == mtmp && levl[mtmp->mx][mtmp->my].typ == POOL) {
	...
	case 'c':
		...
		if (ctmp || !rn2(20) || (flags.moonphase == NEW_MOON && !carrying(DEAD_LIZARD))) {
		    Stoned = 5;
