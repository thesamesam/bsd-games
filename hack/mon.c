...
void justswld(struct monst *mtmp, const char *name)
{
...
void youswld(struct monst *mtmp, int dam, int die, const char *name)
{
    if (mtmp != u.ustuck)
	return;
...
int dochug(struct monst *mtmp)
{
    ...
    // regenerate monsters
    if ((!(moves % 20) || strchr(MREGEN, mdat->mlet)) && mtmp->mhp < mtmp->mhpmax)
	mtmp->mhp++;
	...
	if (cansee(mtmp->mx, mtmp->my) && (!Stealth || (mdat->mlet == 'e' && rn2(10))) && (!strchr("NL", mdat->mlet) || !rn2(50)) && (Aggravate_monster || strchr("d1", mdat->mlet)
																      || (!rn2(7) && !mtmp->mimic)))
	    mtmp->msleep = 0;
	else
	    return 0;
    ...
    scared = (nearby && (sengr_at("Elbereth", u.ux, u.uy) || sobj_at(SCR_SCARE_MONSTER, u.ux, u.uy)));
    if (scared && !mtmp->mflee) {
	mtmp->mflee = 1;
	mtmp->mfleetim = (rn2(7) ? rnd(10) : rnd(100));
    }
    if (!nearby || mtmp->mflee || mtmp->mconf || (mtmp->minvis && !rn2(3)) || (strchr("BIuy", mdat->mlet) && !rn2(4)) || (mdat->mlet == 'L' && !u.ugold && (mtmp->mgold || rn2(2))) || (!mtmp->mcansee && !rn2(4)) || mtmp->mpeaceful) {
	tmp = m_move(mtmp, 0); // 2: monster died moving
	if (tmp == 2 || (tmp && mdat->mmove <= 12))
	    return tmp == 2;
    }
    if (!strchr("Ea", mdat->mlet) && nearby && !mtmp->mpeaceful && u.uhp > 0 && !scared) {
	if (mhitu(mtmp))
...
int m_move(struct monst *mtmp, int after)
{
    ...
    if (msym == 'U' && !mtmp->mcan && canseemon(mtmp) && mtmp->mcansee && rn2(5)) {
	if (!Confusion)
  ...
  not_special:
    ...
    if (mtmp->mflee)
	appr = -1;
    if (mtmp->mconf || Invis || !mtmp->mcansee || (strchr("BIy", msym) && !rn2(3)))
	appr = 0;
    omx = mtmp->mx;
    omy = mtmp->my;
    gx = u.ux;
...
#define	SRCHRADIUS	25
    ...
    nix = omx;
    niy = omy;
    cnt = mfndpos(mtmp, poss, info, msym == 'u' ? NOTONL : (msym == '@' || msym == '1') ? (ALLOW_SSM | ALLOW_TRAPS) : strchr(UNDEAD, msym) ? NOGARLIC : ALLOW_TRAPS);
...
#define	NEW_SCORING
	...
	pline("You destroy %s!", mtmp->mtame ? amonnam(mtmp, "poor") : monnam(mtmp));
    }
    if (u.umconf) {
	if (!Blind)
...
#define	MAXMONNO	100
	...
	u.nr_killed[tmp]++;
	if ((nk = u.nr_killed[tmp]) > MAXMONNO && !strchr(fut_geno, mdat->mlet))
	...
	if (ul < 14)	       // points are given based on present and
		...
		if (u.uexp + 1 + (tmp + ((tmp2 <= 0) ? 0 : 4 << (tmp2 - 1))) / nk >= 10 * pow((unsigned) (ul - 1)))
		    if (++ul == 14)
			break;
...
int newcham(			// make a chameleon look like a new monster
	       // returns 1 if the monster actually changed
	       struct monst *mtmp, const struct permonst *mdat)
{
    int mhp, hpn, hpd;
...
void mnexto(			// Make monster mtmp next to you (if
				// possible)
	       struct monst *mtmp)
{
    coord mm;
