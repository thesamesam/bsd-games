...
int nexttodoor(int sx, int sy)
{
    ...
    for (dx = -1; dx <= 1; dx++)
	for (dy = -1; dy <= 1; dy++)
	    if ((lev = &levl[sx + dx][sy + dy])->typ == DOOR || lev->typ == SDOOR || lev->typ == LDOOR)
		return 1;
    return 0;
}
...
{
    return sroom->lx <= xdnstair && xdnstair <= sroom->hx && sroom->ly <= ydnstair && ydnstair <= sroom->hy;
}

int has_upstairs(struct mkroom *sroom)
{
    return sroom->lx <= xupstair && xupstair <= sroom->hx && sroom->ly <= yupstair && yupstair <= sroom->hy;
}
