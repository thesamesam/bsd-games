...
const struct permonst li_dog = { "little dog", 'd', 2, 18, 6, 1, 6, sizeof(struct edog) };
const struct permonst dog = { "dog", 'd', 4, 16, 5, 1, 6, sizeof(struct edog) };
const struct permonst la_dog = { "large dog", 'd', 6, 15, 4, 2, 4, sizeof(struct edog) };
...
int dogfood(struct obj *obj)
{
    switch (obj->olet) {
	case FOOD_SYM:
	    return (obj->otyp == TRIPE_RATION) ? DOGFOOD : (obj->otyp < CARROT) ? ACCFOOD : (obj->otyp < CORPSE) ? MANFOOD : (poisonous(obj) || obj->age + 50 <= moves || obj->otyp == DEAD_COCKATRICE)
		    ? POISON : CADAVER;
	default:
	    if (!obj->cursed)
		return APPORT;
...
#define DDIST(x,y) ((x-omx)*(x-omx) + (y-omy)*(y-omy))
	...
	if ((obj = o_at(omx, omy)) != NULL)
		...
		if (obj->owt < 10 * mtmp->data->mlevel)
			...
			if (rn2(udist) || !rn2((int) edog->apport)) {
			    freeobj(obj);
			    unpobj(obj);
			    // if(levl[omx][omy].scrsym == obj->olet) newsym(omx,omy);
			    mpickobj(mtmp, obj);
			}
	    }
    }
	...
	if (otyp < MANFOOD && (dogroom >= 0 || DDIST(obj->ox, obj->oy) < 10)) {
	    if (otyp < gtyp || (otyp == gtyp && DDIST(obj->ox, obj->oy) < DDIST(gx, gy))) {
		gx = obj->ox;
		gy = obj->oy;
		gtyp = otyp;
	    }
	} else if (gtyp == UNDEF && dogroom >= 0 && uroom == dogroom && !mtmp->minvent && edog->apport > (unsigned) rn2(8)) {
	    gx = obj->ox;
	    gy = obj->oy;
	    gtyp = APPORT;
	}
    }
    if (gtyp == UNDEF || (gtyp != DOGFOOD && gtyp != APPORT && moves < edog->hungrytime)) {
	...
	if (after && udist <= 4 && gx == u.ux && gy == u.uy)
	    return 0;
	if (udist > 1) {
	    if (!IS_ROOM(levl[u.ux][u.uy].typ) || !rn2(4) || whappr || (mtmp->minvent && rn2((int) edog->apport)))
		appr = 1;
	}
	...
	if (info[i] & ALLOW_M) {
	    ...
	    if (hitmm(mtmp, mtmp2) == 1 && rn2(4) && mtmp2->mlstmv != moves && hitmm(mtmp2, mtmp) == 2)
		return 2;
	    return 0;
	}
	// dog avoids traps
	...
	while (obj) {
	    ...
	    if (obj->cursed)
		goto nxti;
	    if (obj->olet == FOOD_SYM && (otyp = dogfood(obj)) < MANFOOD && (otyp < ACCFOOD || edog->hungrytime <= moves)) {
		// Note: our dog likes the food so much that he might eat it even when it conceals a cursed object
		nix = nx;
		niy = ny;
		chi = i;
	      eatobj:
