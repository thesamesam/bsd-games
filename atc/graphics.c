...
void init_gr(void)
{
    ...
    credit = newwin(INPUT_LINES, PLANE_COLS, LINES - INPUT_LINES, COLS - PLANE_COLS);
    planes = newwin(LINES - INPUT_LINES, PLANE_COLS, 0, COLS - PLANE_COLS);
}

void setup_screen(const C_SCREEN * scp)
{
    int i, j;
    char str[3];
    const char *airstr;
	...
	str[1] = ' ';
	draw_line(radar, scp->line[i].p1.x, scp->line[i].p1.y, scp->line[i].p2.x, scp->line[i].p2.y, str);
    }
...
void draw_line(WINDOW * w, int x, int y, int lx, int ly, const char *s)
{
    int dx, dy;
...
void ioaddstr(int pos, const char *str)
{
    wmove(input, 0, pos);
    waddstr(input, str);
    wrefresh(input);
...
void ioerror(int pos, int len, const char *str)
{
    wmove(input, 1, pos);
    for (int i = 0; i < len; ++i)
	waddch(input, '^');
...
void loser(const PLANE * p, const char *s)
{
    int c;
#ifdef BSD
    struct itimerval itv;
	...
	wprintw(input, "Plane '%c' %s\n\nHit space for top players list...", name(p), s);
    wrefresh(input);
    fflush(stdout);
