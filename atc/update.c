...
void update(int dummy UNUSED)
{
	...
	if (pp->delayd && pp->xpos == sp->beacon[pp->delayd_no].x && pp->ypos == sp->beacon[pp->delayd_no].y) {
	    pp->delayd = 0;
	...
	switch (pp->dest_type) {
	    case T_AIRPORT:
		if (pp->xpos == sp->airport[pp->dest_no].x && pp->ypos == sp->airport[pp->dest_no].y && pp->altitude == 0) {
		...
		break;
	    case T_EXIT:
		if (pp->xpos == sp->exit[pp->dest_no].x && pp->ypos == sp->exit[pp->dest_no].y) {
		    if (pp->altitude != 9)
	...
	if (pp->altitude <= 0) {
		...
		if (pp->xpos == sp->airport[i].x && pp->ypos == sp->airport[i].y) {
		    if (pp->dest_type == T_AIRPORT)
			loser(pp, "landed at the wrong airport.");
		    else
			loser(pp, "landed instead of exited.");
		}
	    loser(pp, "crashed on the ground.");
	}
	if (pp->xpos < 1 || pp->xpos >= sp->width - 1 || pp->ypos < 1 || pp->ypos >= sp->height - 1) {
	    for (i = 0; i < sp->num_exits; i++)
		if (pp->xpos == sp->exit[i].x && pp->ypos == sp->exit[i].y) {
		    if (pp->dest_type == T_EXIT)
			loser(pp, "exited via the wrong exit.");
		    else
	...
	for (p2 = p1->next; p2 != NULL; p2 = p2->next)
		...
		(void) sprintf(buf, "collided with plane '%c'.", name(p2));
		loser(p1, buf);
	    }
...
const char *command(const PLANE * pp)
{
    static char buf[50], *bp, *comm_start;

    buf[0] = '\0';
    bp = buf;
    (void) sprintf(bp, "%c%d%c%c%d: ", name(pp), pp->altitude, (pp->fuel < LOWFUEL) ? '*' : ' ', (pp->dest_type == T_AIRPORT) ? 'A' : 'E', pp->dest_no);
...
{
    if (ABS(p1->altitude - p2->altitude) <= dist && ABS(p1->xpos - p2->xpos) <= dist && ABS(p1->ypos - p2->ypos) <= dist)
	return 1;
    else
	return 0;
}
