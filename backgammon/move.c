...
void move(int okay)		// zero if first move
{
    int i;			// index
    int l;			// last man
...
void trymove(int mvnum, int swapped)	// see if swapped also tested
{
...
static int bcomp(struct BOARD *a, struct BOARD *b)
{
...
void makefree(struct BOARD *dead)	// dead position
{
...
static void boardcopy(struct BOARD *s)	// game situation
{
    int i;			// index
