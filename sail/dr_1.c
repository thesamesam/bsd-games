...
void unfoul(void)
{
	...
	foreachship(to) {
	    if (nat != capship(to)->nationality && !is_toughmelee(sp, to, 0, 0))
		continue;
...
void boardcomp(void)
{
	...
	foreachship(sq) {
	    ...
	    if (meleeing(sp, sq))
		continue;
	    if (!sq->file->dir || sp->nationality == capship(sq)->nationality)
		continue;
...
static int fightitout(struct ship *from, struct ship *to, int key)
{
	...
	if (!menfrom) {	       // if crew surprised
	    if (fromcap == from)
		menfrom = from->specs->crew1 + from->specs->crew2 + from->specs->crew3;
	    else
    ...
    for (count = 0; ((fromstrength < strengthto * 3 && strengthto < fromstrength * 3)
		     || fromstrength == -1) && count < 4; count++) {
	...
	sprintf(message, "killed in melee: %d.  %s: %d", totalto, from->shipname, totalfrom);
	...
	if (key) {
	    if (fromcap != from)
		Write(W_POINTS, fromcap, fromcap->file->points - from->file->struck ? from->specs->pts : 2 * from->specs->pts, 0, 0, 0);
