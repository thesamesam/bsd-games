...
static void prword(const char *const base[], int indx)
{
    printw("%s", base[indx]);
}

static int prwidth(const char *const base[], int indx)
{
...
void showstr(const char *str, int delaysecs)
{
    addstr(str);
    refresh();
    delay(delaysecs * 10);
...
void showboard(const char *b)
{
    tty_showboard(b);
}

void prompt(const char *mesg)
{
...
static void winch_catcher(int signo UNUSED)
{
    // struct winsize win;
    signal(SIGWINCH, winch_catcher);
...
static void tty_showboard(const char *b)
{
    int i;
    int line;
