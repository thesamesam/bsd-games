...
int pickmove (int us)
{
	...
	if (sp->s_occ != EMPTY)
	    continue;
	if (debug && (sp->s_combo[BLACK].c.a == 1 || sp->s_combo[WHITE].c.a == 1)) {
	    sprintf(fmtbuf, "- %s %x/%d %d %x/%d %d %d", stoc(sp - board), sp->s_combo[BLACK].s, sp->s_level[BLACK], sp->s_nforce[BLACK], sp->s_combo[WHITE].s, sp->s_level[WHITE], sp->s_nforce[WHITE], sp->s_wval);
	    dlog(fmtbuf);
	}
    ...
    if (debug) {
	sprintf(fmtbuf, "B %s %x/%d %d %x/%d %d %d", stoc(sp1 - board), sp1->s_combo[BLACK].s, sp1->s_level[BLACK], sp1->s_nforce[BLACK], sp1->s_combo[WHITE].s, sp1->s_level[WHITE], sp1->s_nforce[WHITE], sp1->s_wval);
	dlog(fmtbuf);
	sprintf(fmtbuf, "W %s %x/%d %d %x/%d %d %d", stoc(sp2 - board), sp2->s_combo[WHITE].s, sp2->s_level[WHITE], sp2->s_nforce[WHITE], sp2->s_combo[BLACK].s, sp2->s_level[BLACK], sp2->s_nforce[BLACK], sp2->s_wval);
	dlog(fmtbuf);
...
int better(const struct spotstr *sp, const struct spotstr *sp1, int us)
{
    int them, s, s1;
...
void scanframes (int color)
{
	...
	if (debug) {
	    sprintf(fmtbuf, "%cL%d %d %d %d", "BW"[color], d, combolen - n, combocnt, elistcnt);
	    dlog(fmtbuf);
	    refresh();
	}
	n = combolen;
...
#ifndef NDEBUG
    if (combocnt) {
	sprintf(fmtbuf, "scanframes: %c combocnt %d", "BW"[color], combocnt);
	dlog(fmtbuf);
	whatsup(0);
    }
    if (elistcnt) {
	sprintf(fmtbuf, "scanframes: %c elistcnt %d", "BW"[color], elistcnt);
	dlog(fmtbuf);
	whatsup(0);
    }
#endif
...
void makecombo2 (struct combostr *ocbp, struct spotstr *osp, int off, int s)
{
    struct spotstr *fsp;
    struct combostr *ncbp;
    int f, r, d, c;
	...
	for (f = 0; f < 5; f++, fsp -= d) {	// for each frame
	    ...
	    // make a new combo!
	    ncbp = (struct combostr *) malloc(sizeof(struct combostr) + 2 * sizeof(struct combostr *));
	    if (ncbp == NULL)
		panic("Out of memory!");
		...
		sprintf(fmtbuf, "%c c %d %d m %x %x o %d %d", "bw"[curcolor], ncbp->c_framecnt[0], ncbp->c_framecnt[1], ncbp->c_emask[0], ncbp->c_emask[1], ncbp->c_voff[0], ncbp->c_voff[1]);
		dlog(fmtbuf);
...
// within the frame 'ocbp' and combo value 's'.
void makecombo (struct combostr *ocbp, struct spotstr *osp, int off, int s)
{
...
#ifndef NDEBUG
	    if (sp->s_occ != EMPTY) {
		sprintf(fmtbuf, "loop: %c %s", "BW"[curcolor], stoc(sp - board));
		dlog(fmtbuf);
		whatsup(0);
	    }
#endif
	...
	// make a new combo!
	ncbp = (struct combostr *) malloc(sizeof(struct combostr) + (cbp->c_nframes + 1) * sizeof(struct combostr *));
	if (ncbp == NULL)
	    panic("Out of memory!");
	scbpp = (struct combostr **) (ncbp + 1);
	...
	if (verts) {
		...
		n = (vertices[0].o_intersect - ocbp->c_vertex) / dd[ocbp->c_dir];
	...
	if ((c == 1 && debug > 1) || debug > 3) {
	    sprintf(fmtbuf, "%c v%d i%d d%d c %d %d m %x %x o %d %d", "bw"[curcolor], verts, ncbp->c_frameindex, ncbp->c_dir, ncbp->c_framecnt[0], ncbp->c_framecnt[1], ncbp->c_emask[0], ncbp->c_emask[1], ncbp->c_voff[0], ncbp->c_voff[1]);
	    dlog(fmtbuf);
	    printcombo(ncbp, fmtbuf);
	    dlog(fmtbuf);
	}
...
void makeempty (struct combostr *ocbp)
{
    ...
    *--cbpp = cbp->c_link[0];
	...
	for (s = 0, m = 1; s < 5; s++, sp += d, m <<= 1) {
	    ...
	    if (debug > 2) {
		sprintf(fmtbuf, "e %s o%d i%d c%d m%x %x", stoc(sp - board), nep->e_off, nep->e_frameindex, nep->e_framecnt, nep->e_emask, nep->e_fval.s);
		dlog(fmtbuf);
	    }
...
void updatecombo (struct combostr *cbp, int color)
{
    struct spotstr *sp;
...
void appendcombo (struct combostr *cbp, int color UNUSED)
{
...
int checkframes (struct combostr *cbp, struct combostr *fcbp, struct spotstr *osp, int s, struct ovlp_info *vertices)
{
	...
	if (mask & (1 << n)) {
		...
		// ended frame.
		if ((flg & C_OPEN_1) && (n == tcbp->c_vertex || n == tcbp->c_vertex + 5 * dd[tcbp->c_dir]))
		    return -1;	// invalid overlap
		if (cb.c.b && (n == fcbp->c_vertex || n == fcbp->c_vertex + 5 * dd[fcbp->c_dir]))
		    return -1;	// invalid overlap

		vertices->o_intersect = n;
		vertices->o_fcombo = cbp;
		vertices->o_link = 1;
		vertices->o_off = (n - tcbp->c_vertex) / dd[tcbp->c_dir];
	...
	if (osp != &board[n]) {
	    ...
	    // one of the end points if it is an open
	    // ended frame.
	    if ((flg & C_OPEN_0) && (n == cbp->c_vertex || n == cbp->c_vertex + 5 * dd[cbp->c_dir]))
		return -1;   // invalid overlap
	    if (cb.c.b && (n == fcbp->c_vertex || n == fcbp->c_vertex + 5 * dd[fcbp->c_dir]))
		return -1;   // invalid overlap
	    ...
	    vertices->o_link = 0;
	    vertices->o_off = (n - cbp->c_vertex) / dd[cbp->c_dir];
...
int sortcombo (struct combostr **scbpp, struct combostr **cbpp, struct combostr *fcbp)
{
	...
	sprintf(fmtbuf, "sortc: %s%c l%d", stoc(fcbp->c_vertex), pdir[fcbp->c_dir], curlevel);
	dlog(fmtbuf);
	str = fmtbuf;
	for (cpp = cbpp; cpp < cbpp + curlevel; cpp++) {
	    sprintf(str, " %s%c", stoc((*cpp)->c_vertex), pdir[(*cpp)->c_dir]);
	    str += strlen(str);
	}
	dlog(fmtbuf);
    }
...
void printcombo (struct combostr *cbp, char *str)
{
    sprintf(str, "%x/%d", cbp->c_combo.s, cbp->c_nframes);
    str += strlen(str);
    for (struct combostr *tcbp; (tcbp = cbp->c_link[1]) != NULL; cbp = cbp->c_link[0]) {
	sprintf(str, " %s%c%x", stoc(tcbp->c_vertex), pdir[tcbp->c_dir], cbp->c_flg);
	str += strlen(str);
    }
...
#ifndef NDEBUG
void markcombo (struct combostr *ocbp)
{
    struct combostr *cbp, *tcbp, **cbpp;
    struct elist *ep, *nep;
    struct spotstr *sp;
...
void clearcombo (struct combostr *cbp, int open)
{
    struct spotstr *sp;
    struct combostr *tcbp;
    int d, n, mask;
...
int list_eq (struct combostr **scbpp, struct combostr **cbpp, int n)
{
    struct combostr **spp, **cpp;
