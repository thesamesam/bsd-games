...
void calcmove(void)
{
    ...
    unsigned i, count200, badcount, nummin, nummax, diff;
    int curmin, curmax;
    CARD safe, oppos;
	...
	switch (card) {
	    ...
	    case C_SPARE_SAFE:
	    case C_RIGHT_WAY:
		if (pp->battle == opposite(card) || (pp->speed == C_LIMIT && card == C_RIGHT_WAY)) {
		    Movetype = M_PLAY;
		    Card_no = i;
		    return;
		}
	...
	if (is_safety(pp->hand[i])) {
	    ...
	    if (Numseen[oppos] == Numcards[oppos] && !(pp->hand[i] == C_RIGHT_WAY && Numseen[C_LIMIT] != Numcards[C_LIMIT]))
		goto playsafe;
	    else if (!cango && (op->can_go || !pp->can_go || Topcard < Deck)) {
  ...
  redoit:
    foundlow = (cango || count[C_END_LIMIT] != 0 || Numseen[C_LIMIT] == Numcards[C_LIMIT]
	...
	if (is_safety(card) || playit[i] == (cango != 0)) {
		...
		case C_50:
		    ...
		    if (Topcard > Deck && cango && diff <= 100 && (int) diff / Value[card] > count[card]
			&& (card == C_25 || diff % 50 == 0)) {
			if (card == C_50 && diff - 50 == 25 && count[C_25] > 0)
			    goto okay;
			*value = 0;
			if (--cango <= 0)
			    goto redoit;
		  ...
		  okay:
		    *value = (Value[card] >> 3);
			...
			-- * value;
		    if (!foundlow && (card == C_50 || count[C_50] == 0)) {
		...
		case C_END_LIMIT:
			...
			*value = 1;
		    else if (pp->speed == C_LIMIT || Numseen[C_LIMIT] != Numcards[C_LIMIT]) {
			safe = S_RIGHT_WAY;
			oppos = C_LIMIT;
			goto repair;
		    } else {
		...
		case C_GAS:
			...
			*value = (pp->safety[safe] == S_PLAYED ? -1 : 1);
		    else if (pp->battle != oppos && (Numseen[oppos] == Numcards[oppos] || Numseen[oppos] + count[card] > Numcards[oppos])) {
			*value = 0;
			--count[card];
		    } else {
		      repair:
		...
		case C_GO:
			...
			*value = (pp->safety[S_RIGHT_WAY] == S_PLAYED ? -1 : 2);
		    else if (pp->can_go && Numgos + count[C_GO] == Numneed[C_GO]) {
			*value = 0;
			--count[C_GO];
		    } else {
		...
		case C_STOP:
			...
			*value *= Numcards[C_STOP] + Numseen[C_GO];
			...
			if (op->mileage == 0)
			    *value += 5;
			if ((card == C_LIMIT && op->speed == C_LIMIT) || !op->can_go)
			    *value -= 5;
...
int onecard (const PLAY *pp)
{
    ...
    spd = pp->speed;
    card = -1;
    if (pp->can_go || ((is_repair(bat) || bat == C_STOP || spd == C_LIMIT) && Numseen[S_RIGHT_WAY] != 0) || (bat >= 0 && Numseen[safety(bat)] != 0))
...
int canplay (const PLAY* pp, const PLAY* op, CARD card)
{
    switch (card) {
	case C_200:
	    if (pp->nummiles[C_200] == 2)
	...
	case C_STOP:
	    ...
	    break;
	case C_LIMIT:
	    if (op->speed != C_LIMIT && op->safety[S_RIGHT_WAY] != S_PLAYED && op->mileage + 50 < End)
		return true;
	    break;
	case C_GAS:
	case C_SPARE:
	case C_REPAIRS:
	    ...
	    break;
	case C_GO:
	    if (!pp->can_go && (is_repair(pp->battle) || pp->battle == C_STOP))
		return true;
	    break;
	case C_END_LIMIT:
	    if (pp->speed == C_LIMIT)
