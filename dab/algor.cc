...
// algor.cc: Computer algorithm
#include "algor.h"
#include "board.h"
#include "box.h"
#include "random.h"

ALGOR::ALGOR (const char c)
: PLAYER(c)
{
#ifdef notyet
    // Single Edges = (x + y) * 2
...
int ALGOR::find_closure(size_t& y, size_t& x, int& dir, BOARD& b)
{
	...
	for (x = rdx(); x < b.nx(); x = rdx()) {
		...
		for (dir = BOX::first; dir < BOX::last; dir++)
		    if (!box.isset(dir))
			return 1;
		b.abort("find_closure: 3 sided box[%d,%d] has no free sides", y, x);
	    }
	}
    }
    return 0;
...
// Count a closure, by counting all boxes that we can close in the current move
...
int ALGOR::try_good_turn (BOX& box, size_t y, size_t x, int& dir, BOARD& b)
{
    ...
    if (box.count() >= 2)
	b.abort("try_good_turn: box[%d,%d] has more than 2 sides occupied", y, x);
...
int ALGOR::try_bad_turn (BOX& box, size_t& y, size_t& x, int& dir, BOARD& b, int last)
{
    if (4 - box.count() <= last)
	b.abort("try_bad_turn: Called at [%d,%d] for %d with %d", y, x, last, box.count());
    for (dir = BOX::first; dir < BOX::last; ++dir) {
	if (!box.isset(dir)) {
	    if (!last)
		return 1;
...
int ALGOR::find_bad_turn (size_t& y, size_t& x, int& dir, BOARD& b, int last)
{
	...
	for (x = rdx(); x < b.nx(); x = rdx()) {
	    BOX box(y, x, b);
	    if ((4 - box.count()) > last && try_bad_turn(box, y, x, dir, b, last))
		return 1;
	}
    }
    return 0;
...
int ALGOR::find_min_closure1 (size_t& y, size_t& x, int& dir, const BOARD& b, int last)
{
    BOARD nb(b);
	...
	if ((mv = nb.domove(ty, tx, tdir, getWho())) != 0)
	    b.abort("find_min_closure1: Invalid move %d (%d, %d, %d)", mv, ty, tx, tdir);
