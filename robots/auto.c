...
static COORD *closest_robot(dist)
int *dist;
{
...
//      and put in dist its distance
static COORD *closest_heap(dist)
int *dist;
{
...
static char move_towards(int dx, int dy)
{
...
static char move_between(rob, hp)
COORD *rob;
COORD *hp;
{
    int dx, dy;
    ...
    CONSDEBUG(("me (%d,%d) robot(%d,%d) heap(%d,%d) delta(%d,%d)", My_pos.x, My_pos.y, rob->x, rob->y, hp->x, hp->y, dx, dy));
    return move_towards(dx, dy);
}			       // end move_between

// between():
//      Return true if the heap is between us and the robot
int between(COORD* rob, COORD* hp)
{
    // I = @
...
char automove(void)
{
    ...
    robot_heap = distance(robot_close->x, robot_close->y, heap_close->x, heap_close->y);
