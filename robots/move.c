...
// Must I teleport; i.e., is there anywhere I can move without being eaten?
...
bool do_move (int dy, int dx)
{
    ...
    if (newpos.y <= 0 || newpos.y >= Y_FIELDSIZE || newpos.x <= 0 || newpos.x >= X_FIELDSIZE || Field[newpos.y][newpos.x] > 0 || eaten(&newpos)) {
	if (Running) {
	    Running = false;
	    leaveok(stdscr, false);
