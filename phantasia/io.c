...
int getanswer (const char* choices, bool def)
{
    int ch;			// input
    ...
    for (loop = 3; loop; --loop) { // try 3 times
	if (setjmp(Timeoenv) != 0) { // timed out waiting for response
	    if (def || loop <= 1) // return default answer
		break;
	    else // prompt, and try again
		goto YELL;
	} else {
	    // wait for response
	    clrtoeol();
	    ...
	    if (ch < 0) { // caught some signal
		++loop;
		continue;
	    } else if (ch == CH_REDRAW) { // redraw screen
	    ...
	    if (def || strchr(choices, ch) != NULL) // valid choice
		return ch;
	    else if (!def && loop > 1) { // bad choice; prompt, and try again
...
void catchalarm(int dummy UNUSED)
{
    longjmp(Timeoenv, 1);
}
