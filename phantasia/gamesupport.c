...
void changestats (bool ingameflag)
{
	...
	mvprintw(18, 8, "4:Palantir %c  5:Blessing %c  6:Virgin %c  7:Blind %c", flag[(int) playerp->p_palantir], flag[(int) playerp->p_blessing], flag[(int) playerp->p_virgin], flag[(int) playerp->p_blindness]);

	if (!Wizard)
	    mvprintw(19, 8, "8:Ring    %c", flag[playerp->p_ring.ring_type != R_NONE]);
	else
	    mvprintw(19, 8, "8:Ring    %d  9:Duration %d", playerp->p_ring.ring_type, playerp->p_ring.ring_duration);
	...
	if (Wizard)
	    c = getanswer(" ", true);
	else // examining own player; allow to change name and password
	...
	switch (c) {
		    ...
		    // get new name
		{
		    ...
		    if (!Wizard)
			Echo = false;
		    do { // get two copies of new password until they match
			// get first copy
	    ...
	    case '!':	       // quit, update
		if (Wizard && (!ingameflag || playerp != &Player))
	...
	continue;
      ...
      BALTER:
	mvprintw(23, 0, "%s = %c; %s = ", prompt, flag[(int) *bptr], prompt);
	c = getanswer("\nTF", true);
	if (c == 'T')
	    *bptr = true;
	else if (c == 'F')
...
void monstlist(void)
{
	...
	printf("%2d)  %-20.20s%4.0f   %4.0f     %2.0f   %5.0f  %5.0f     %2d    %2d     %3.0f\n", count++, Curmonster.m_name, Curmonster.m_strength, Curmonster.m_brains, Curmonster.m_speed, Curmonster.m_energy, Curmonster.m_experience,
...
void activelist(void)
{
	...
	if (Other.p_status != S_NOTUSED)
	    printf("%-20s   (%-9s)  Level: %6.0f  %s  (%s)\n", Other.p_name, Other.p_login, Other.p_level, descrtype(&Other, false), descrstatus(&Other));
