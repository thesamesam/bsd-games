...
const char* descrlocation (struct player* playerp, bool shortflag)
{
...
void tradingpost(void)
{
    ...
    // print Menu
    move(7, 0);
    for (loop = 0; loop < size; ++loop) { // print Menu
	if (loop == 6)
	    cost = blessingcost;
	else
	    cost = Menu[loop].cost;
	...
	printw("Shield: %9.0f  Sword: %9.0f  Quicksilver:%3.0f  Blessed: %s\n", Player.p_shield, Player.p_sword, Player.p_quksilver, (Player.p_blessing ? " True" : "False"));
	...
	switch (ch) {
		...
		if (ch - '0' > size)
		    ...
		    switch (ch) {
			case '1':
			    printw("Mana is one per %.0f gold piece.  How many do you want (%.0f max) ? ", Menu[0].cost, floor(Player.p_gold / Menu[0].cost));
			...
			case '2':
			    printw("Shields are %.0f per +1.  How many do you want (%.0f max) ? ", Menu[1].cost, floor(Player.p_gold / Menu[1].cost));
			...
			case '3':
			    printw("A book costs %.0f gp.  How many do you want (%.0f max) ? ", Menu[2].cost, floor(Player.p_gold / Menu[2].cost));
				...
				if (drandom() < 0.02)
				    dishonest = true;
				else if (drandom() * numitems > Player.p_level / 10.0 && numitems != 1) {
				    printw("\nYou blew your mind!\n");
				    Player.p_brains /= 5;
				} else {
			...
			case '4':
			    printw("Swords are %.0f gp per +1.  How many + do you want (%.0f max) ? ", Menu[3].cost, floor(Player.p_gold / Menu[3].cost));
			...
			case '5':
			    printw("A charm costs %.0f gp.  How many do you want (%.0f max) ? ", Menu[4].cost, floor(Player.p_gold / Menu[4].cost));
			...
			case '6':
			    printw("Quicksilver is %.0f gp per +1.  How many + do you want (%.0f max) ? ", Menu[5].cost, floor(Player.p_gold / Menu[5].cost));
	    ...
	    case 'S':	       // sell gems
		mvprintw(15, 0, "A gem is worth %.0f gp.  How many do you want to sell (%.0f max) ? ", (double) N_GEMVALUE, Player.p_gems);
	...
	if (cheat == 1)
	    ...
	    refresh();
	    altercoordinates(Player.p_x - Player.p_x / 10.0, Player.p_y - Player.p_y / 10.0, A_SPECIFIC);
	    sleep(2);
	    return;
	}
    }
...
void displaystats(void)
{
    ...
    mvprintw(1, 0, "Level :%7.0f   Energy  :%9.0f(%9.0f)  Mana :%9.0f  Users:%3d\n", Player.p_level, Player.p_energy, Player.p_maxenergy + Player.p_shield, Player.p_mana, Users);
    mvprintw(2, 0, "Quick :%3.0f(%3.0f)  Strength:%9.0f(%9.0f)  Gold :%9.0f  %s\n", Player.p_speed, Player.p_quickness + Player.p_quksilver, Player.p_might, Player.p_strength + Player.p_sword, Player.p_gold, descrstatus(&Player));
}
...
void allstatslist(void)
{
    ...
    mvprintw(18, 0, "Blessing: %s   Ring: %s   Virgin: %s   Palantir: %s", flags[(int) Player.p_blessing], flags[Player.p_ring.ring_type != R_NONE], flags[(int) Player.p_virgin], flags[(int) Player.p_palantir]);
}

const char* descrtype (struct player* playerp, bool shortflag)
{
	...
	case SC_NONE: type = playerp->p_type; break;
	case SC_KING: type = 7; break;
	case SC_COUNCIL: type = 8; break;
	case SC_EXVALAR: type = 9; break;
	case SC_VALAR: type = 10; break;
    }
    ...
    if (shortflag) // use short descriptions
	++type;
...
long findname(const char *name, struct player *playerp)
{
...
void freerecord(struct player *playerp, long loc)
{
...
void death(const char *how)
{
	    ...
	    // extra lives
	{
	    ...
	    Player.p_specialtype = SC_COUNCIL;
	    return;
	} else if (Player.p_ring.ring_inuse && (Player.p_ring.ring_type == R_DLREG || Player.p_ring.ring_type == R_NAZREG))
	    ...
	    // from death
	{
		...
		--Player.p_crowns;
	    return;
	} else if (Player.p_ring.ring_type == R_BAD || Player.p_ring.ring_type == R_SPOILED)
	    // bad ring in
	    // possession; name
	    // idiot after player
	{
    ...
    fprintf(fp, "%s (%s, run by %s, level %.0f, killed by %s)", Player.p_name, descrtype(&Player, true), Player.p_login, Player.p_level, how);
    fclose(fp);
...
void writerecord(struct player *playerp, long place)
{
...
double explevel(double experience)
{
    if (experience < 1.1e7)
...
void altercoordinates(double xnew, double ynew, int operation)
{
    switch (operation) {
...
void readrecord(struct player *playerp, long loc)
{
...
void adjuststats(void)
{
    ...
    dtemp = ((Player.p_gold + Player.p_gems / 2.0) - 1000.0) / Statptr->c_goldtote - Player.p_level;
    ...
    Player.p_mana = MIN(Player.p_mana, Player.p_level * Statptr->c_maxmana + 1000.0);
    Player.p_brains = MIN(Player.p_brains, Player.p_level * Statptr->c_maxbrains + 200.0);
...
void initplayer(struct player *playerp)
{
    playerp->p_experience = playerp->p_level = playerp->p_strength = playerp->p_sword = playerp->p_might = playerp->p_energy = playerp->p_maxenergy = playerp->p_shield = playerp->p_quickness = playerp->p_quksilver = playerp->p_speed = playerp->p_magiclvl =
	playerp->p_mana = playerp->p_brains = playerp->p_poison = playerp->p_gems = playerp->p_sin = playerp->p_1scratch = playerp->p_2scratch = 0.0;
    ...
    playerp->p_lives = playerp->p_crowns = playerp->p_charms = playerp->p_amulets = playerp->p_holywater = playerp->p_lastused = 0;
    playerp->p_status = S_NOTUSED;
    playerp->p_tampered = T_OFF;
    playerp->p_istat = I_OFF;

    playerp->p_palantir = playerp->p_blessing = playerp->p_virgin = playerp->p_blindness = false;

    playerp->p_name[0] = playerp->p_password[0] = playerp->p_login[0] = '\0';
}
...
const char *descrstatus(struct player *playerp)
{
