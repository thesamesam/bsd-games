...
int main(int argc, char **argv)
{
    ...
    while (--argc && (*++argv)[0] == '-') {
	switch ((*argv)[1]) {
	    case 's': noheader = true; break;		// short
	    case 'H': headeronly = true; break;		// Header
	    case 'a': activelist(); cleanup(true);	// all users
	    case 'p': purgeoldplayers(); cleanup(true);	// purge old players
	    case 'S': Wizard = !getuid(); break;	// set 'Wizard'
	    case 'x': examine = true; break;		// examine
	    case 'm': monstlist(); cleanup(true);	// monsters
	    case 'b': scorelist(); cleanup(true);	// scoreboard
	}
    }
    ...
    do { // get the player structure filled
	Fileloc = -1L;
	mvaddstr(22, 17, "Do you have a character to run [Q = Quit] ? ");
	switch (getanswer("NYQ", false)) {
	    case 'Y': Fileloc = recallplayer(); break;
	    case 'Q': cleanup(true);
	    default: Fileloc = rollnewplayer(); break;
	}
	clear();
    }
    while (Fileloc < 0L);

    if (Player.p_level > 5.0) // low level players have long timeout
	Timeout = true;
    ...
    for (;;) { // loop forever, processing input
	adjuststats();	       // cleanup stats
	if (Throne && Player.p_crowns == 0 && Player.p_specialtype != SC_KING) { // not allowed on throne -- move
	    mvaddstr(5, 0, "You're not allowed in the Lord's Chamber without a crown.\n");
	    altercoordinates(0.0, 0.0, A_NEAR);
	}
	checktampered();       // check for energy voids, etc.

	if (Player.p_status != S_CLOAKED // not cloaked
	    && (dtemp = fabs(Player.p_x)) == fabs(Player.p_y) // |x| = |y|
	    && !Throne) { // not on throne
	    dtemp = sqrt(dtemp / 100.0);
	    if (floor(dtemp) == dtemp) { // |x| / 100 == n*n; at a trading post
		tradingpost();
		clear();
	    }
	}
	...
	if (Player.p_status == S_CLOAKED) {
		...
		Player.p_mana -= 3.0;
	    else { // ran out of mana, uncloak
		Player.p_status = S_PLAYING;
		Changed = true;
	    }
	}

	if (Player.p_status != S_PLAYING && Player.p_status != S_CLOAKED) { // change status back to S_PLAYING
	    Player.p_status = S_PLAYING;
	    Changed = true;
	}
	if (Changed) { // update file only if important stuff has changed
	    writerecord(&Player, Fileloc);
	    Changed = false;
	    continue;
	}
	...
	move(6, 0);
	if (Throne)	// maybe make king, print prompt, etc.
	    throneroom();
...
long rollnewplayer(void)
{
	...
	// print out results
	mvprintw(12, 14, "Strength    :  %2.0f  Quickness:  %2.0f  Mana       :  %2.0f\n", Player.p_strength, Player.p_quickness, Player.p_mana);
	mvprintw(13, 14, "Energy Level:  %2.0f  Brains   :  %2.0f  Magic Level:  %2.0f\n", Player.p_energy, Player.p_brains, Player.p_magiclvl);
    ...
    {
	mvprintw(18, 0, "Give your character a name [up to %d characters] ?  ", SZ_NAME - 1);
...
void procmain(void)
{
	...
	default:	       // rest
	    Player.p_energy += (Player.p_maxenergy + Player.p_shield) / 15.0 + Player.p_level / 3.0 + 2.0;
	    Player.p_energy = MIN(Player.p_energy, Player.p_maxenergy + Player.p_shield);
...
void titlelist(void)
{
    ...
    if ((fp = fopen(_PATH_MOTD, "r")) != NULL && fgets(Databuf, SZ_DATABUF, fp) != NULL) {
	...
	if (Other.p_specialtype == SC_KING && Other.p_status != S_NOTUSED)
	    // found the king
	{
	    sprintf(Databuf, "The present ruler is %s  Level:%.0f", Other.p_name, Other.p_level);
	...
	} else if (Other.p_experience > nxtexp && Other.p_specialtype <= SC_KING && Other.p_status != S_NOTUSED)
    ...
    sprintf(Databuf, "%s  Level:%.0f   and   %s  Level:%.0f", hiname, hilvl, nxtname, nxtlvl);
    mvaddstr(17, 40 - strlen(Databuf) / 2, Databuf);

    // print last to die
    if ((fp = fopen(_PATH_LASTDEAD, "r")) != NULL && fgets(Databuf, SZ_DATABUF, fp) != NULL) {
...
void neatstuff(void)
{
	...
	default:	       // deal with poison
	    if (Player.p_poison > 0.0) {
		temp = Player.p_poison * Statptr->c_weakness * Player.p_maxenergy / 600.0;
		if (Player.p_energy > Player.p_maxenergy / 10.0 && temp + 5.0 < Player.p_energy)
...
void genchar(int type)
{
    ...
    Player.p_quickness = ROLL(statptr->c_quickness.base, statptr->c_quickness.interval);
    Player.p_strength = ROLL(statptr->c_strength.base, statptr->c_strength.interval);
    Player.p_mana = ROLL(statptr->c_mana.base, statptr->c_mana.interval);
    Player.p_maxenergy = Player.p_energy = ROLL(statptr->c_energy.base, statptr->c_energy.interval);
    Player.p_brains = ROLL(statptr->c_brains.base, statptr->c_brains.interval);
    Player.p_magiclvl = ROLL(statptr->c_magiclvl.base, statptr->c_magiclvl.interval);
