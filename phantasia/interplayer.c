...
void checkbattle(void)
{
	...
	if (Other.p_status != S_OFF && Other.p_status != S_NOTUSED && Other.p_status != S_HUNGUP && (Other.p_status != S_CLOAKED || Other.p_specialtype != SC_VALAR)) {
	    // player is on and not a cloaked valar
	    ++Users;
	    if (Player.p_x == Other.p_x && Player.p_y == Other.p_y
		// same coordinates
		&& foeloc != Fileloc
		// not self
		&& Player.p_status == S_PLAYING && (Other.p_status == S_PLAYING || Other.p_status == S_INBATTLE)
		// both are playing
		&& Other.p_specialtype != SC_VALAR && Player.p_specialtype != SC_VALAR) {
		// neither is valar
		battleplayer(foeloc);
		return;
	    }
...
void battleplayer(long foeplace)
{
...
void checktampered(void)
{
	...
	if (Enrgyvoid.ev_active && Enrgyvoid.ev_x == Player.p_x && Enrgyvoid.ev_y == Player.p_y) {
	    // sitting on one
	    if (loc > 0L) {
...
void tampered(int what, double arg1, double arg2)
{
	...
	case T_GRAIL:
		...
		Player.p_mana += 1000;
	    } else if (Player.p_specialtype == SC_VALAR || Player.p_specialtype == SC_EXVALAR) {
		...
		if (drandom() / 2.0 < Player.p_sin) {
		    ...
		    Player.p_strength = Player.p_mana = Player.p_energy = Player.p_maxenergy = Player.p_magiclvl = Player.p_brains = Player.p_experience = Player.p_quickness = 1.0;
...
void userlist (bool ingameflag)
{
    ...
    while (fread((char *) &Other, SZ_PLAYERSTRUCT, 1, Playersfp) == 1) {
	if (Other.p_status == S_NOTUSED // record is unused
	    || (Other.p_specialtype == SC_VALAR && Other.p_status == S_CLOAKED)) { // cloaked valar
	    if (!Wizard)
	...
	if (ingameflag &&
	    ...
	    && (Other.p_status != S_CLOAKED || (Player.p_specialtype == SC_VALAR && Player.p_palantir))
	    // not cloaked; valar can see through cloak with a palantir
	    && Other.p_specialtype != SC_VALAR)
	    // not a valar
	    // coordinates should be printed
	    printw("%-20s  %8.0f  %8.0f ", Other.p_name, Other.p_x, Other.p_y);
	else
	    // cannot see player's coordinates
	    printw("%-20s %19.19s ", Other.p_name, descrlocation(&Other, true));

	printw("%6.0f %s  %-9.9s%s\n", Other.p_level, descrtype(&Other, true), Other.p_login, descrstatus(&Other));
...
void dotampered(void)
{
	...
	if ((loc = findname(Databuf, &Other)) >= 0L) {
		...
		if (tamper == T_RELOCATE && CIRCLE(temp1, temp2) < CIRCLE(Other.p_x, Other.p_y)
		    && !Wizard)
		    mvaddstr(5, 0, "Cannot move someone closer to the Lord's Chamber.\n");
		else {
		    if (tamper == T_BESTOW)
			Player.p_gold -= floor(temp1);
		    if (!Wizard && (tamper == T_HEAL || tamper == T_MONSTER || tamper == T_RELOCATE || tamper == T_BLESSED))
...
void writevoid(struct energyvoid *vp, long loc)
{
