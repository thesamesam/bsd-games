...
void encounter (int particular)
{
	...
	for (;;) {
	    ...
	    if (Curmonster.m_type == SM_DARKLORD && Player.p_blessing && Player.p_charms > 0) {
	    ...
	    if (drandom() * Curmonster.m_speed > drandom() * Player.p_speed // monster is faster
		&& Curmonster.m_type != SM_DARKLORD // not D. L.
		&& Curmonster.m_type != SM_SHRIEKER // not mimic
		&& !firsthit)
		monsthits();	// monster gets a hit
	    ...
	    if (Lines > LINES - 2) { // near bottom of screen - pause
		more(Lines);
		move(Lines = 8, 0);
		clrtobot();
	    }
	...
	++flockcnt;
	longjmp(Fightenv, 0);
    } else if (Circle > 1.0 && Curmonster.m_treasuretype > 0 && drandom() > 0.2 + pow(0.4, (double) (flockcnt / 3 + Circle / 3.0))) {
...
void playerhits(void)
{
	...
	case '1':	       // melee
	    ...
	    Curmonster.m_strength = Curmonster.m_o_strength - Curmonster.m_melee / Curmonster.m_o_energy * Curmonster.m_o_strength / 4.0;
	    hitmonster(inflict);
	    break;
	...
	case '2':	       // skirmish
	    ...
	    Curmonster.m_maxspeed = Curmonster.m_o_speed - Curmonster.m_skirmish / Curmonster.m_o_energy * Curmonster.m_o_speed / 4.0;
	    hitmonster(inflict);
	    break;
	...
	case '3':	       // evade
	    ...
	    if ((Curmonster.m_type == SM_DARKLORD || Curmonster.m_type == SM_SHRIEKER
		 ...
		 || drandom() * Player.p_speed * Player.p_brains > drandom() * Curmonster.m_speed * Curmonster.m_brains)
	...
	case '5':	       // nick
	    ...
	    {
		mvprintw(Lines++, 0, "You hit %s %.0f times, and made him mad!", Enemyname, inflict);
	...
	case '6':	       // luckout
		...
		{
		    if ((drandom() + 0.333) * Player.p_brains < (drandom() + 0.333) * Curmonster.m_brains)
...
void monsthits(void)
{
	...
	case SM_SHRIEKER:
	    // call a big monster
	    mvaddstr(Lines++, 0, "Shrieeeek!!  You scared it, and it called one of its friends.");
	    more(Lines);
	...
	case SM_BALROG:
	    ...
	    mvprintw(Lines++, 0, "%s took away %.0f experience points.", Enemyname, inflict);
	...
	default:
		...
		case SM_LEANAN:
		    ...
		    mvprintw(Lines++, 0, "%s sapped %.0f of your strength!", Enemyname, inflict);
		...
		case SM_SARUMAN:
			...
			// gems turn to gold
		    {
			mvprintw(Lines++, 0, "%s transformed your gems into gold!", Enemyname);
		...
		case SM_VORTEX:
		    ...
		    mvprintw(Lines++, 0, "%s sucked up %.0f of your mana!", Enemyname, inflict);
		...
		case SM_NAZGUL:
		    ...
		    mvprintw(Lines++, 0, "%s neutralized 1/5 of your brain!", Enemyname);
		    Player.p_brains *= 0.8;
		    break;

		case SM_TIAMAT:
		    // take some gold and gems
		    mvprintw(Lines++, 0, "%s took half your gold and gems and flew off.", Enemyname);
		    Player.p_gold /= 2.0;
		    Player.p_gems /= 2.0;
		    cancelmonster();
		    break;
		...
		case SM_KOBOLD:
		    ...
		    mvprintw(Lines++, 0, "%s stole one gold piece and ran away.", Enemyname);
		...
		case SM_SHELOB:
		    ...
		    mvprintw(Lines++, 0, "%s has bitten and poisoned you!", Enemyname);
		    Player.p_poison -= 1.0;
		    break;
		...
		case SM_SMEAGOL:
			...
			// try to steal ring
		    {
			mvprintw(Lines++, 0, "%s tried to steal your ring, ", Enemyname);
			if (drandom() > 0.1)
		...
		case SM_SUCCUBUS:
		    ...
		    mvprintw(Lines++, 0, "%s sapped %.0f of your energy.", Enemyname, inflict);
		    Player.p_energy -= inflict;
		    break;

		case SM_CERBERUS:
		    // take all metal treasures
		    mvprintw(Lines++, 0, "%s took all your metal treasures!", Enemyname);
		    Player.p_crowns = 0;
		    Player.p_sword = Player.p_shield = Player.p_gold = 0.0;
		    cancelmonster();
		    break;

		case SM_UNGOLIANT:
		    // (large) poison and take a quickness
		    mvprintw(Lines++, 0, "%s poisoned you, and took one quik.", Enemyname);
		    Player.p_poison += 5.0;
		...
		case SM_JABBERWOCK:
		    ...
		    // Bonnacon
		    mvprintw(Lines++, 0, "%s flew away, and left you to contend with one of its friends.", Enemyname);
		...
		case SM_TROLL:
		    ...
		    mvprintw(Lines++, 0, "%s partially regenerated his energy.!", Enemyname);
		    Curmonster.m_energy += floor((Curmonster.m_o_energy - Curmonster.m_energy) / 2.0);
...
void hitmonster(double inflict)
{
	    ...
	    // all other types of monsters
	{
	    ...
	    if (Curmonster.m_type == SM_MIMIC && strcmp(Curmonster.m_name, "A Mimic") != 0 && !Player.p_blindness)
...
void throwspell(void)
{
	...
	switch (ch) {
		...
		if (Player.p_magiclvl < ML_INCRMIGHT)
		    ...
		    Player.p_might += (1.2 * (Player.p_strength + Player.p_sword)
		...
		if (Player.p_magiclvl < ML_INVISIBLE)
		    ...
		    Player.p_speed += (1.2 * (Player.p_quickness + Player.p_quksilver)
		...
		if (Player.p_magiclvl < ML_XPORT)
		    ...
		    if (Player.p_brains + Player.p_magiclvl < Curmonster.m_experience / 200.0 * drandom()) {
		...
		if (Player.p_magiclvl < ML_PARALYZE)
		    ...
		    if (Player.p_magiclvl > Curmonster.m_experience / 1000.0 * drandom()) {
...
void callmonster(int which)
{
	    ...
	    // randomize some stats
	{
	    ...
	    Curmonster.m_treasuretype = (int) ROLL(0.0, (double) Curmonster.m_treasuretype);
	} else
	    // make Modnar into Morgoth
	{
	    strcpy(Curmonster.m_name, "Morgoth");
	    Curmonster.m_strength = drandom() * (Player.p_maxenergy + Player.p_shield) / 1.4 + drandom() * (Player.p_maxenergy + Player.p_shield) / 1.5;
	    Curmonster.m_brains = Player.p_brains;
	    Curmonster.m_energy = Player.p_might * 30.0;
	    Curmonster.m_type = SM_MORGOTH;
	    Curmonster.m_speed = Player.p_speed * 1.1 + ((Player.p_specialtype == SC_EXVALAR) ? Player.p_speed : 0.0);
	    Curmonster.m_flock = 0.0;
	    Curmonster.m_treasuretype = 0;
	    Curmonster.m_experience = 0.0;
	}
...
void awardtreasure(void)
{
	    ...
	    // gems
	{
	    ...
	    // gold
	{
	    gold = ROLL(treasuretype * 10.0, treasuretype * treasuretype * 10.0 * (Circle - 1.0));
	...
	if (ch != 'Y')
		...
		case 1:       // treasure type 1
			...
			case 2:
			    ...
			    Player.p_experience += ROLL(0.0, 2000.0 + Circle * 400.0);
			    break;
		...
		case 4:       // treasure type 4
			...
			switch ((int) ROLL(1, 6)) {
				...
				more(whichtreasure);
				Shield = (Player.p_maxenergy + Player.p_energy) * 5.5 + Circle * 50.0;
				Whichmonster = pickmonster();
				longjmp(Fightenv, 0);
				// NOTREACHED
		...
		case 13:      // treasure types 10 - 13
			...
			if (Curmonster.m_treasuretype == 10) {
			    ...
			    Player.p_quickness += 2.0;
			    break;
			} else if (Curmonster.m_treasuretype == 11 && !Player.p_palantir) {
			    addstr("You've acquired Saruman's palantir.\n");
			    Player.p_palantir = true;
			    break;
			} else if (Player.p_ring.ring_type == R_NONE && Player.p_specialtype < SC_COUNCIL && (Curmonster.m_treasuretype == 12 || Curmonster.m_treasuretype == 13))
			    // roll
			    //  up
			    // a
			    // ring
		...
		case 9:       // treasure type 9
		    switch (whichtreasure) {
			case 1:
			    if (Player.p_level <= 1000.0 && Player.p_crowns <= 3 && Player.p_level >= 10.0) {
