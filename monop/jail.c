...
// This routine uses a get-out-of-jail-free card to get the player out of jail.
void card (void)
{
    if (cur_p->loc != JAIL) {
...
// This routine returns the players get-out-of-jail-free card to a deck.
...
void printturn(void)
{
	...
	case 2:	printf("3rd (and final)"); break;
    }
    printf(" turn in JAIL)\n");
}
