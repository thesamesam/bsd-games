...
void cleanupboard(void)
{
	...
	for (ptr = tableau[cnt], row = tabrow; ptr != NIL; ptr = ptr->next, row++)
	    removecard(col, row);
    }
}
...
void printrank (int a, int b, const struct cardtype* cp, bool inverse)
{
    move(b, a);
    if (cp->rank != 10)
	addch(' ');
...
void printcard(int a, int b, const struct cardtype *cp)
{
    if (cp == NIL)
	removecard(a, b);
...
bool notempty (const struct cardtype* cp)
{
    if (cp == NIL) {
	errmsg = true;
	move(msgrow, msgcol);
...
bool ranklower (const struct cardtype* cp1, const struct cardtype* cp2)
{
    if (cp2->rank == Ace)
	if (cp1->rank == King)
	    return true;
...
bool diffcolor (const struct cardtype* cp1, const struct cardtype* cp2)
{
...
bool tabok (const struct cardtype* cp, int des)
{
    if ((cp == stock) && (tableau[des] == NIL))
	return true;
    else if (tableau[des] == NIL)
	if (stock == NIL && cp != bottom[0] && cp != bottom[1] && cp != bottom[2] && cp != bottom[3])
	    return true;
	else
	    return false;
...
void showstat(void)
{
    ...
    for (row = coldrow, col = coldcol, ptr = talon; ptr != NIL; ptr = ptr->next) {
    ...
    for (row = cnewrow, col = cnewcol, ptr = hand; ptr != NIL; ptr = ptr->next) {
	if (ptr->paid == false && ptr->visible == true) {
	    ptr->paid = true;
	    this.information += costofinformation;
	    game.information += costofinformation;
...
void clearstat(void)
{
    ...
    move(stockstatrow, stockstatcol - 7);
    printw("          ");
    for (int row = ctoprow; row <= cbotrow; row++) {
	move(row, cinitcol);
	printw("%56s", " ");
    }
}
...
void updatebettinginfo(void)
{
    ...
    thiscosts = this.hand + this.inspection + this.game + this.runs + this.information + this.thinktime;
    gamecosts = game.hand + game.inspection + game.game + game.runs + game.information + game.thinktime;
    totalcosts = total.hand + total.inspection + total.game + total.runs + total.information + total.thinktime;
    ...
    printw("%4ld%8ld%9ld", this.inspection, game.inspection, total.inspection);
    ...
    printw("%4ld%8ld%9ld", this.information, game.information, total.information);
...
bool rankhigher (const struct cardtype* cp, int let)
{
...
int samesuit(const struct cardtype *cp, int let)
{
...
void movetofound(struct cardtype **cp, int source)
{
	...
	do {
		...
		if (rankhigher(*cp, tempbase)
		    ...
		    printcard(pilemap[tempbase], foundrow, found[tempbase]);
		    timesthru = 0;
		    if (mtforigin == stk) {
			usedstock();
			printcard(stockcol, stockrow, stock);
...
void getcmd(int row, int col, const char *cp)
{
    char cmd[2] = { '\0', '\0' }, ch;
    int i;
...
void movecard(void)
{
	...
	if (destpile >= '1' && destpile <= '4')
	    dest = (int) (destpile - '1');
	if (!startedgame && (srcpile == 't' || srcpile == 's' || srcpile == 'h' || srcpile == '1' || srcpile == '2' || srcpile == '3' || srcpile == '4')) {
	    startedgame = true;
	    osrcpile = srcpile;
	    odestpile = destpile;
	    if (status != BETTINGBOX)
	...
	switch (srcpile) {
		...
		if (status == BETTINGBOX) {
		    do {
			getcmd(moverow, movecol, "Buy game?");
		    } while (srcpile != 'y' && srcpile != 'n');
		    if (srcpile == 'n') {
			showcards();
			done = true;
...
void initall (void)
{
    ...
    if (dbfd < 3)
	exit(1);
    if (0 > lseek(dbfd, uid * sizeof(struct betinfo), SEEK_SET)) {
	close(dbfd);
	dbfd = -1;
	return;
    }
    if (0 > read(dbfd, (char *) &total, sizeof(total))) {
	close(dbfd);
	dbfd = -1;
	return;
    }
